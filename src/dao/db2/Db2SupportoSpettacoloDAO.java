...
public class Db2SupportoSpettacoloDAO implements SupportoSpettacoloDAO {
	...
	private static final String READ_ALL_TO_APPROVE = "SELECT * " + " FROM " + TABLE + " " + "WHERE " + FK_ID_NON_APPROVATO + " IS NOT NULL";
	...
	public List<SupportoSpettacolo> readAllSpettacoliDaRevisionare() {
		...
		Connection conn = Db2DAOFactory.createConnection();
		try {
			PreparedStatement prep_stmt = conn.prepareStatement(READ_ALL_TO_APPROVE);
			System.out.println(READ_ALL_TO_APPROVE);
			prep_stmt.clearParameters();
			ResultSet rs = prep_stmt.executeQuery();
			...
			System.err.println("read(): failed to retrieve shows that need review");
			e.printStackTrace();
		} finally {
			Db2DAOFactory.closeConnection(conn);
		}
